library(bigsnpr)
library(dplyr)
ukb <- snp_attach("data/UKBB_hm3.rds")
G     <- ukb$genotypes
...
bigassertr::assert_dir("data/corr_hm3_small")
...
all_final_grp <- future_map_dfr(1:22, function(chr) {
  ...
  corr0 <- runonce::save_run(
    snp_cor(G, ind.row = sort(sample(nrow(G), 2000)), ind.col = ind.chr,
            infos.pos = POS2[ind.chr], size = 3 / 1000, ncores = NCORES),
    file = paste0("data/corr_hm3_small/chr", chr, ".rds")
  )
  ...
  }, file = paste0("data/corr_hm3_small/LD_with_blocks_chr", chr, ".rds"))
...
sum(all_final_grp$n_block) # 783
plot(all_final_grp$cost)
plot(all_final_grp$cost2)
# saveRDS(all_final_grp, "data/corr_hm3_small/all_final_grp.rds")

sum(file.size(paste0("data/corr_hm3_small/LD_with_blocks_chr", 1:22, ".rds"))) /
  sum(file.size(paste0("data/corr_hm3_small/chr", 1:22, ".rds")))
# 76%
