...
As covariates, we first recompute PCs for this subset of individuals using function \texttt{snp\_autoSVD} from R package bigsnpr and keep four PCs based on visual inspection \cite[]{prive2017efficient,prive2020efficient}. We also use sex (Field 22001), age (Field 21022), birth date (combining Fields 34 and 52) and deprivation index (Field 189) as additional covariates.
...
We design two other LD matrices: one using a subset of 2000 individuals from the previously selected ones (which we call ``hm3\_small''), and one based on 10,000 individuals from around South Europe by using the ``Italy'' center defined in \cite{prive2021high} (``hm3\_altpop'').
...
This results in an extended set of 1,444,196 SNPs, of which we compute the correlation between variants and apply the optimal algorithm developed in \cite{prive2021optimal} to obtain 431 independent LD blocks.
