...
Finally, LDpred2-auto can be used to reliably infer the predictive performance $r^2$ of its resulting polygenic score, directly from within the Gibbs sampler, even when power is low (Figure \ref{fig:simu_r2}).

We also run simulations with binary outcomes where the simulated continuous liabilities are transformed to binary outcomes using a thresholding corresponding to the prevalence.
...
We then investigate previous estimates using the extended set of HapMap3 variants proposed here (Methods). As expected, compared to HapMap3, higher $h^2$ (average increase of 12.3\% [10.8-13.7]) and lower $p$ (decrease of 11.5\% [10.7-12.3]) estimates are obtained with this extended set HapMap3+ (Figure \ref{fig:hm3_plus}). 
...
Back to the UKBB data, instead of computing per-block heritability estimates, we compute per-annotation ones, to investigate functional enrichment. We perform this analysis for height using 50 non-flanking binary annotations from the baselineLD v2.2 model \cite[]{finucane2018heritability}.
Heritability enrichments are rather modest, ranging from 0.7 to 2.5 with a GWAS sample size of N=305K, and of slightly smaller magnitude with N=100K (Figure \ref{fig:enrichment}).

Across 169 UKBB phenotypes with more than 25 chains kept, we compute the median heritability per block, and compare it to the number of variants in these blocks in Figure \ref{fig:median_local_h2}. The median heritability explained by a block of variants is largely proportional to the number of variants in this block.
Unsurprisingly, the outlier block explaining a much larger heritability contains the HLA region (Figure \ref{fig:median_local_h2}).
...
Some variants in multiple small regions across the genome seem to have a much larger probability of being causal; these are mapped to genes that are known to be associated with many different traits (up to more than 300) in the GWAS Catalog \cite[]{buniello2019nhgri}.
