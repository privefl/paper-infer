...
Here we extend LDpred2-auto to make it a highly reliable method for estimating $h^2$ (global and local), $p$ (also per-variant probability for fine-mapping purposes), and $\alpha$.
...
We then investigate using the extended set of HapMap3 variants proposed here, HapMap3+, which includes \textasciitilde40\% more variants on top of HapMap3 variants recommended to use for LDpred2 (Methods). As expected, compared to HapMap3, higher $h^2$ (average increase of 12.3\% [95\% CI: 10.8, 13.7]) and lower $p$ (decrease of 11.5\% [10.7, 12.3]) estimates are obtained with this extended set HapMap3+ (Figure \ref{fig:hm3_plus}). 
...
We first infer the genetic ancestry proportions of individuals included in the meta-analysis using the method proposed in
\cite{prive2021using}, and find that 81.9\% are from N.W. Europe, 9.5\% from E. Europe, 6.5\% from Finland, 1.5\% of Ashkenazi genetic ancestry, 0.3\% from S.W. Europe, and 0.2\% from W. Africa. 
...
This means that this extended SNP set is still not tagging all variants perfectly, and that it might be preferable to use a more targeted set of variants for phenotypes for which most of the heritability is contained in a single region of the genome.
