...
These methods include GCTA ($h^2$, \cite{yang2011gcta}), BOLT-REML ($h^2$ and $p$, \cite{loh2015contrasting}), LD Score regression ($h^2$, \cite{bulik2015ld}), FINEMAP (per-variant $p$, also called posterior inclusion probabilities PIP, used for fine-mapping, \cite{benner2016finemap}), HESS (local $h^2$, \cite{shi2016contrasting}), LDAK-SumHer ($h^2$, \cite{speed2019sumher}), S-LD4M ($p$, \cite{oconnor2019extreme}), GRM-MAF-LD ($\alpha$, \cite{schoech2019quantification}), SuSiE (PIP, \cite{wang2020simple}), SBayesS ($h^2$, $p$, and a third parameter $S$, similar to $\alpha$, \cite{zeng2021widespread}), and BEAVR (local $p$, \cite{johnson2021estimation}).

As previously shown by \cite{daetwyler2008accuracy}, $h^2$ and $p$ can also be used to determine how well we can predict a phenotype from using genetic variants alone, where $r^2_\text{max} = h^2 \cdot r^2_{g\hat{g}} = \dfrac{h^2}{1 + (1 - r^2_\text{max}) \frac{M p}{N h^2}}$ ($N$ is the sample size, $M$ is the number of variants, so $M p$ is the number of causal variants). 
...
[TODO: MAKE SURE TO CITE / INCLUDE ALL NEW FIGURES]
...
Then we keep only chains that provide top imputed marginal scaled effect sizes $\check{\beta} = R \bar{\beta}$, where $R$ is the correlation matrix and $\bar{\beta}$ are the PGS scaled effect sizes [TODO: MOVE TO ITS OWN METHOD SECTION, ALSO INTRODUCING NEW AND PREVIOUS].

First, LDpred2-auto generally reliably infers the three parameters from its model, i.e.\ the SNP heritability $h^2$, polygenicity $p$, and $\alpha$ (Figures \ref{fig:simu_h2} and \ref{fig:simu_h2_large}--\ref{fig:simu_alpha_large}).
Compared to LD Score regression, heritability estimates are as precise when power is low, and much more precise when power is large, especially for small polygenicity values (Figures \ref{fig:simu_h2} and \ref{fig:simu_h2_large}).
When power is low (e.g.\ $N=20000$ and $h^2=0.01)$, LDpred2\_noMLE (with only two model parameters, as in previous versions of LDpred2-auto) and SBayesS are both over-confident in their estimate of the heritability (i.e.\ CIs are small, and do not contain the true parameter value), and LDpred2\_jump overestimates the heritability (Figure \ref{fig:simu_h2}).
...
The $\alpha$ estimate of LDpred2-auto (with the new 3-parameter model) can become very unprecise when power is too low, which can be detected by a small number of chains kept from LDpred2-auto, while estimates from SBayesS are often over-confident with small CIs that do not include the true simulated values (Figure \ref{fig:simu_alpha}).
Finally, we also investigate new ways of post-filtering chains in LDpred2-auto, for both prediction and inference (``LDpred2\_newfilter'', Methods); results remain practically unchanged (Figures \ref{fig:simu_h2} and \ref{fig:simu_h2_large}--\ref{fig:simu_alpha_large}).
...
Finally, LDpred2-auto can also be used to reliably infer the predictive performance $r^2$ of its resulting polygenic score, directly from within the Gibbs sampler (Methods), even when power is low (Figure \ref{fig:simu_r2}). We show it works with results from SBayesS's Gibbs sampler as well (Figure \ref{fig:simu_r2}).

\begin{figure}[p]
	\centerline{\includegraphics[width=0.85\textwidth]{est_h2_one_20K}}
...
\begin{figure}[!htb]
	...
	\caption{Inferred predictive performance $r^2$ from the Gibbs sampler of LDpred2-auto versus the ones obtained in the test set, for all 248 phenotypes defined from the UK Biobank. These are stratified by the polygenicity estimated from LDpred2-auto. Green dashed lines represent the 1:1 line. The 95\% confidence interval for the LDpred2-auto estimate is obtained from the 2.5\% and 97.5\% quantiles of all the $r^2$ estimates from the iterations (after burn-in) of the chains kept. The 95\% confidence interval for $r^2$ in the test set is obtained from bootstrap. ``F\_height'' and ``M\_height'' use females and males only, respectively (in both GWAS and test sets). \label{fig:ukbb_r2}}
\end{figure}
...
Interestingly, using the previous approach (\texttt{use\_MLE = FALSE}) seems to provide more robust results, where we can get chains to converge (and therefore non-zero predictive performance) for the 7 (3 + 4) phenotypes mentioned before when using the previous alternative LD references (Figure \ref{fig:compare_MLE}).
...
Here we run three LDpred2-auto models for height, one based on 100K UKBB individuals (as a random subset of the 356K used before for which height is recorded), one from the same 305K UKBB individuals used before, and one from a large GWAS meta-analysis of 1.6M individuals of European genetic ancestries across 1,373,020 variants \cite[]{yengo2022saturated}.
...
Intercepts from LD Score regression are increasing with sample size: 1.02 (SE: 0.008) with N=100K, 1.11 (0.015) with N=305K, and 2.31 (0.068) with N=1.6M, which is unfortunately expected \cite[]{loh2018mixed}.
...
We emphasize that, even though there are 1.6M individuals in the meta-analysis, the predictive performance corresponds to around 87\% of the SNP heritability only, therefore an even larger sample size is required to be able to better predict height.
...
These results are similar irrespective of whether ``allow\_jump\_sign'' is used or not, which is surprising to us.
...
A description of the eight external GWAS summary statistics used is provided in Table \ref{tab:sumstats}; these were not partly derived from UKBB individuals.
...
Because of the increased mismatch between external GWAS summary statistics and the LD reference we use (compared to simulations and UKBB analyses), we also explore multiple values for parameter \texttt{coef\_shrink}, which is a multiplicative coefficient for shrinking/regularizing off-diagonal elements of the LD matrix in LDpred2-auto.
...
However, results are virtually unchanged when regularizing the LD matrix (``hm3\_plus\_regul'') by multiplying correlations between variants $i$ and $j$ by $\exp(-0.5 \cdot d_{i,j})$, where $d_{i,j}$ is the distance in cM between the two variants (similarly to as proposed in \cite{wen2010using}), which is a bit surprising to us.
...
Since we use individuals of diverse ancestries for computing pairwise variant correlations that are used to construct this extended set of variants, we expect this new set to be beneficial across diverse ancestries.
...
This can also be computed for a specific chain by taking two samples $\boldsymbol{\beta_1}$ and $\boldsymbol{\beta_2}$ that are far enough on the same chain to remove the possible autocorrelation. This is what we use for SBayesS here, and also as an alternative means for post-filtering chains for prediction (cf.\ next Methods section).
...
Because running a Gibbs sampler with so many variants and with possible mismatches between e.g.\ the GWAS summary statistics and the LD reference used, we have always recommended to run multiple chains in LDpred2-auto, and post-filter some of them as quality control \cite[]{prive2020ldpred2}.
...
Moreover, here we test two other filtering criteria in first simulations based on continuous outcomes (and call this ``LDpred2\_newfilter''). 
First, for prediction, we test to filter on the average of $\boldsymbol{\beta_1}^T \boldsymbol{R} \boldsymbol{\beta_2}$, which is an estimate of $r^2$ (cf.\ previous Methods section).
Second, for inference, we filter on some convergence criterion. The split-Rhat statistic is a popular metric to test for good mixing and convergence of Markov chains \cite[]{vehtari2021rank}. We have found this statistic to perform badly when e.g.\ one parameter get stuck and is constant; in this case, the chain do not mix well, but we do get a perfect Rhat of 1.0. However, we have found that a two-sample Cramer-von Mises statistic \cite[]{anderson1962distribution}, by similarly using both parts of the chain after burn-in, is often highly correlated with the split-Rhat while not suffering from the previous issue.
