...
\begin{figure}[!htb]
	\centerline{\includegraphics[width=0.82\textwidth]{overview}}
...
Then the new implementation of LDpred2-auto is used (Methods).
...
However, running LDpred2 with this extended set of variants takes around 50\% more time; yet, we recall that LDpred2 has been made much faster in \cite{prive2021identifying}, and now runs in less than one hour for 50 chains parallelized over 13 cores (Figure \ref{fig:ukbb_runtimes}), instead of 4--12 hours before.
...
Moreover, the $h^2$ from LDpred2-auto is also slightly overestimated when using a small LD reference panel or when the reference panel does not closely match with the ancestry of the GWAS summary statistics.
...
We now sample $p$ from $\text{Beta}(1 + M_c / \bar{l^2}, 1 + (M - M_c) / \bar{l^2})$, where $\bar{l^2}$ is the average LD score, to add more sampling by properly accounting for the reduced effective number of variants.
...
These are compared to the partial-$r^2$ (on individual-level data from a separate test set). The partial correlation (and 95\% CI) is computed with function \texttt{pcor} from R package bigstatsr, adjusting for the same eight covariates as in the GWAS, then squared (while keeping the sign).
...
All code used for this paper is available at \url{https://github.com/privefl/paper-infer/tree/master/code} [TODO: UPDATE README].
Descriptions of UKBB phenotypes used here are available at \url{https://github.com/privefl/paper-infer/blob/main/phenotype-description.tsv}.
