...
In this model, the expected variance explained by a genetic variant is proportional to $\big[f(1-f)\big]^\alpha$, where $f$ is the allele frequency of this variant.
...
CIs for this $r^2$ estimate very often encompass the true simulated value, expect for LDpred2-auto\_jump when $h^2$ and $N$ are small (Figures \ref{fig:simu10_r2} and \ref{fig:simu10_r2_large}).
...
Here, if not otherwise specified, we use options \texttt{use\_MLE = TRUE} (i.e.\ the new 3-parameter model and sampling scheme) and \texttt{allow\_jump\_sign = FALSE}.
...
Then, we perform a GWAS for these variants using the same training individuals as used before (Figure \ref{manhattan_height}).
...
Heritability enrichments that we obtain from LDpred2-auto results are rather modest, ranging from 0.7 to 2.5 with a GWAS sample size of N=305K, and of slightly smaller magnitude with N=100K and slightly larger magnitude with N=1.6M (Figure \ref{fig:enrichment}).
...
Our intuition and conclusion is that using more variants is beneficial when power is sufficient; however, when power is low (e.g.\ small $N$, small $h^2$, and/or large $p$), it may be actually be detrimental.
...
Therefore, we recommend to use the 3-parameter model (with $\alpha$, when using \texttt{use\_MLE = TRUE}), but only when power is sufficient.
...
For now, one can concentrate on small regions of the genome, where LDpred2-auto can be rerun using a much more dense set of variants, as typically done in fine-mapping analyses (cf.\ Results section ``Local heritability and polygenicity'').
