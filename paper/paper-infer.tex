...
We then investigate previous estimates using the extended set of HapMap3 variants proposed here (Methods). As expected, compared to HapMap3, higher $h^2$ and lower $p$ estimates are obtained with this extended set HapMap3+ (Figure \ref{fig:hm3_plus}). 
A much larger $h^2$ estimate is obtained for lipoprotein(a) concentration [TODO: ADD NUMBERS], which is also reflected in a larger predictive performance.
[TODO: COMPUTE AVERAGE R2 BOOST FOR OTHER PHENO]
