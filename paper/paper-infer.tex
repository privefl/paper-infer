...
As previously shown by \cite{daetwyler2008accuracy}, $h^2$ and $p$ can also be used to determine how well we can predict a phenotype from using genetic variants alone, where $r^2_\text{max} = \dfrac{h^2}{1 + (1 - r^2_\text{max}) \frac{M p}{N h^2}}$ ($N$ is the sample size, $M$ is the number of variants, so $M p$ is the number of causal variants). 
...
Then, we run a GWAS to obtain summary statistics using $N$ individuals (either the 200,000 dedicated to this, or a subset of 20,000), using fast linear regressions implemented in \texttt{big\_univLinReg} from R package bigstatsr \cite[]{prive2017efficient}.
...
The $\alpha$ estimate of LDpred2-auto (with the new 3-parameter model) can become unreliable or unprecise when power is too low, which can be detected by a small number of chains kept from LDpred2-auto, while estimates from SBayesS are often over-confident with small CIs that do not include the true simulated values (Figure \ref{fig:simu_alpha}).
...
In terms of predictive performance, using the HapMap3+ variants provides equal or better predictive performance compared to using the HapMap3 variants, except for type 1 diabetes (T1D); it seems more useful to use this new set of variants for larger sample sizes.
...
Others have proposed to estimate $r^2$ from a reference genotype set \cite[]{mak2017polygenic} or from an additional set of external GWAS summary statistics and LD reference \cite[]{witteveen2022publicly}; here we only use the summary statistics that we input to LDpred2-auto.
