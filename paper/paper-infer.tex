...
As previously shown by \cite{daetwyler2008accuracy}, $h^2$ and $p$ can also be used to determine how well we can predict a phenotype from using genetic variants alone, with $r^2_\text{max} = h^2 \cdot r^2_{g\hat{g}} = \dfrac{h^2}{1 + (1 - r^2_\text{max}) \frac{M p}{N h^2}}$, where $N$ is the sample size, $M$ is the number of variants, so $M p$ is the number of causal variants. 
...
We call `HapMap3+' this extended set of 1,444,196 SNPs, and recommend to use it when power is sufficient.
...
\begin{table}[htb]
	...
	\begin{tabular}{|l||c|c|c||c|c|}
		...
		\hline
	\end{tabular}
	\caption{Summary of external GWAS summary statistics used. These do not include UKBB individuals. Note that some of them may contain a substantial amount of non-European genetic ancestry (e.g.\ 20\%). \label{tab:sumstats}}
\end{table}
...
Since we used individuals of diverse ancestries for computing the pairwise variant correlations used for constructing this extended set of variants, we expect this new set of variants to be beneficial across diverse ancestries.
...
Finally, in both models and sampling schemes now implemented in LDpred2-auto, we now detect strong divergence when $\boldsymbol{\beta}^T \boldsymbol{\beta} > 2 \boldsymbol{\hat{\beta}}^T \boldsymbol{\hat{\beta}}$, where $\boldsymbol{\beta}$ is the scaled effect sizes from one iteration of the Gibbs sampler and $\boldsymbol{\hat{\beta}}$ is the marginal scaled effect sizes; corresponding chains are stopped and missing values are returned for effect sizes and estimates of missing iterations.
...
Moreover, here we test two other filtering criteria in the first simulations based on continuous outcomes (and call this ``LDpred2\_newfilter''). 
...
However, we have found this statistic to perform badly when e.g.\ one parameter gets stuck and is constant; in this case, the chain do not mix well, but we do get a perfect Rhat of 1.0. 
Instead, we have found that a two-sample Cramer-von Mises statistic \cite[]{anderson1962distribution}, by similarly using both parts of the chain after burn-in, is often highly correlated with the split-Rhat while not suffering from the previous issue.
